      ...
      if(image.img1&&productname&&brand&&rating&&price&&description&&year&&hp){
          if(price>0){
            if(hp>0){
              CalPrice();
              setLoading(true)
              await axios.put('/api/crudproduct',data)
              .then(res=>{
                res=res.data
                if(res.message==='Product Successfully Updated'){
                ...
                }else if(res.message==="Product upload Failed"){
...
return (
    <div className='pb-6 pt-1'>
      <h1 className='text-center mytxt font-semibold mt-6 px-6 text-lg'>Edit Product</h1>
          ...
          <div className="grid gap-6 mb-6  ">
            ...
            {/* PRODUCT NAME */}
              ...
              <div className='flex justify-center'>
                <Button type='submit'  style={{maxWidth:'25rem',width:'100%',backgroundColor:'#00779C'}} variant='contained' color='success'>
                {loading ? <div className='w-ful h-full flex justify-center'><MoonLoader color='white' size={20}/></div>:<span>Update Product</span>}
                </Button>
            </div>
          </div>
      </form>
